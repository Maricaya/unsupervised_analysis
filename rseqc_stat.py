...
def run_rseqc_stat(sample, unit):
    input_bam = f"results/star/{sample}_{unit}/Aligned.sortedByCoord.out.bam"
    output_file = f"results/qc/rseqc/{sample}_{unit}.stats.txt"
    log_file = f"logs/rseqc/rseqc_stat/{sample}_{unit}.log"
    ...
    print(f"Finished processing rseqc_stat for {sample} {unit}")
