...
def run_rseqc_readgc(sample, unit):
    input_bam = f"results/star/{sample}_{unit}/Aligned.sortedByCoord.out.bam"
    output_file = f"results/qc/rseqc/{sample}_{unit}.readgc.GC_plot.pdf"
    log_file = f"logs/rseqc/rseqc_readgc/{sample}_{unit}.log"
    
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    os.makedirs(os.path.dirname(log_file), exist_ok=True)

    prefix = output_file.replace(".GC_plot.pdf", "")
    rseqc_cmd = [
        "read_GC.py",
        "-i", input_bam,
        "-o", prefix
    ...
    print(f"Finished processing rseqc_readgc for {sample} {unit}")
