...
def run_rseqc_junction_annotation(sample, unit):
    input_bam = f"results/star/{sample}_{unit}/Aligned.sortedByCoord.out.bam"
    annotation_bed = "results/qc/rseqc/annotation.bed"
    output_file = f"results/qc/rseqc/{sample}_{unit}.junctionanno.junction.bed"
    log_file = f"logs/rseqc/rseqc_junction_annotation/{sample}_{unit}.log"
    
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
    os.makedirs(os.path.dirname(log_file), exist_ok=True)

    prefix = output_file.replace(".junction.bed", "")
    rseqc_cmd = [
    ...
    print(f"Finished processing rseqc_junction_annotation for {sample} {unit}")
...
for sample in units.index.get_level_values("sample_name").unique():
    for unit in units.loc[sample].index.get_level_values("unit_name").unique():
        run_rseqc_junction_annotation(sample, unit)
